...
get RNAseq info
```{r}
rnaexp1 <- read_excel("../../v3.0annotation/20170617-samples/input/wy001_RNAseq.xlsx") %>%
...
get leaf data
```{r}
leafexp_density <- read_excel("density_exp_leaf_ln.xlsx", sheet = 2) %>%
  filter(density=="CR")
leafexp_density
```
...
get RNAseq info
```{r}
rnaexp_density <- read_csv("../../v3.0annotation/20180202-samples/input/20180202-data/wy003-metadata.csv") %>%
  filter(CROWDING=="CR") %>%
  rename_all(tolower)
rnaexp_density
```
...
### rootnames
```{r}
rootnames <- readLines("../../v3.0annotation/20180202-samples/output/voom_expression.e1and3.resid.root.txt.gz", n=1) %>%
  str_split("\t") %>%
...
```{r}
leafnames <- readLines("../../v3.0annotation/20180202-samples/output/voom_expression.e1and3.resid.leaf.txt.gz", n=1) %>%
  str_split("\t") %>%
  unlist() %>% magrittr::extract(-1)
leafnames
```
...
write_csv(leafrna, "leaf_lengths_combined.csv")
```
