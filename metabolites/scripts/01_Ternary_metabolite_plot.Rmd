...
met_lm %>% group_by(tissue, type) %>%
  mutate(all_trt_lmer_sig0.05 = trt_lmer.pval < 0.05 | `trt:genotype_lmer.pval` < 0.05) %>%
  summarize(trt_or_interaction_lmer_sig_p_0.05 = sum(all_trt_lmer_sig0.05))
...
ggsave("../../_Paper_Figures/Figure_6.png", height = 6.5, width = 6.5)
ggsave("/Volumes/GoogleDrive/Shared drives/BrassicaMicrobeRNAseq/Figs and Tables/Figure_6.png", height = 6.5, width = 6.5)
