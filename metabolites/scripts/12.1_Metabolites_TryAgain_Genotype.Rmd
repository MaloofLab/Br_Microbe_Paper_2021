...
met_per_mg_lm_results %>% 
  summarize(across(starts_with("FDR"), ~ sum(.x < .1)), both=sum(FDR_trt_trtlive < 0.1 & FDR_leaf_value_scale < 0.1), count=n())
...
met_per_mg_lm_results %>% 
  ...
  mutate(sig.both = FDR_trt_trtlive < 0.1 & FDR_leaf_value < 0.1) %>%
  arrange(desc(sig.both), min(.$FDR_trt_trtlive, .$FDR_leaf_value)) %>% 
