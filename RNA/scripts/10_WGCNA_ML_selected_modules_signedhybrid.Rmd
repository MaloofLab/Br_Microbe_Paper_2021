...
GSEA_reports.DF %>% mutate(NAME=NAME %>% str_remove_all(".VOOM5.SIGNEDHYBRID") %>% str_to_lower()) %>% inner_join(ElasticNet0.5_multiCV_Pct_var,by=c("NAME"="tissue.module")) #%>% View()
# write csv
...
genes.count.leaf <- GSEA_cust.leaf %>% 
  ...
  group_by(name) %>% drop_na() %>% summarise(n=n()) %>% 
  dplyr::rename(NAME=name)
genes.count <- bind_rows(genes.count.root,genes.count.leaf) %>% mutate(NAME=str_remove(NAME,".voom5.module")) 
```
...
missing.categories <- ElasticNet0.5_multiCV_Pct_var %>% dplyr::select(tissue.module) %>% filter(!tissue.module %in% WGCNA.voom5.signedhybrid.all.modules.leaf_avg_std.correlated.customcat.GOseq.2.selected$NAME) %>% dplyr::rename(NAME=tissue.module)
...
WGCNA.voom5.signedhybrid.all.modules.leaf_avg_std.correlated.customcat.GOseq.2.selected2 %>% group_by(NAME,category3.au.up_down) %>% summarize(n()) %>% View() # none, why?
