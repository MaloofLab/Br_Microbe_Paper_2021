...
nonElasticNet0.5_multiCV_Pct_var.module <- tibble(all = colnames(d)[-1]) %>% separate(all, c("module","tissue")) %>% mutate(module=str_remove(module,"ME")) %>% unite(tissue.module.all,tissue,module,sep=".") %>% anti_join(ElasticNet0.5_multiCV_Pct_var,by=c("tissue.module.all"="tissue.module")) %>% as_vector()
# 
mod <- ElasticNet0.5_multiCV_Pct_var.module[3]
# making expression pattern plot function (compare within/outside module)
expression.module4 <- function(target.module=mod,dat.voom=root.voom5.mod,tissue_MEs=rootMEs) { 
...
# modify for new ME
temp.target.module <- tibble(tissue_MEs=colnames(tissue_MEs)[-1]) %>% mutate(tissue_MEs = str_remove(tissue_MEs,"^ME")) %>% separate(tissue_MEs,c("module","tissue")) %>% unite(tissue.module,tissue,module,sep=".") %>% as_vector()
# change format of colnames of tissue_MEs
colnames(tissue_MEs)[-1] <- temp.target.module
# get sample levels at target.module
sample.levels <- tissue_MEs %>% dplyr::select(sampleID, quo_name(target.module)) %>% arrange_at(.vars=2) %>% dplyr::select(sampleID) %>% as_vector()
# add expression data 
...
expression.module4()
# 
plot.root.list4 <- tibble(tissue_MEs=colnames(d)[-1]) %>% mutate(tissue_MEs = str_remove(tissue_MEs,"^ME")) %>% separate(tissue_MEs,c("module","tissue")) %>% unite(tissue.module,tissue,module,sep=".") %>% filter(str_detect(tissue.module,"^root")) %>% split(.$tissue.module) %>% map(~expression.module4(target.module=as.name(.$tissue.module),dat.voom=root.voom5.mod,tissue_MEs=rootMEs)) # takes minutes

plot.leaf.list4 <- tibble(tissue_MEs=colnames(d)[-1]) %>% mutate(tissue_MEs = str_remove(tissue_MEs,"^ME")) %>% separate(tissue_MEs,c("module","tissue")) %>% unite(tissue.module,tissue,module,sep=".") %>% filter(str_detect(tissue.module,"^leaf")) %>% split(.$tissue.module) %>% map(~expression.module4(target.module=as.name(.$tissue.module),dat.voom=leaf.voom5.mod,tissue_MEs=leafMEs)) # takes minutes
