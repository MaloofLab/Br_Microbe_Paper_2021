...
=====================
```{r}
sample.description.e1 <- read_csv(file.path("..","output","Br.mbio.e1.sample.description.csv"))
sample.description.e3 <- read_csv(file.path("..","output","Br.mbio.e3.sample.description.csv"))
...
Live.Dead <- ifelse(root.voom5.sample$trt=="live",0,1)
# writing cls file
cat(paste(Total.root.plusR500,"2","1","\n"),file=file.path("..","output","root.voom5.cls"))
cat(paste(description,"\n"),file=file.path("..","output","root.voom5.cls"),append=TRUE)
cat(Live.Dead,file=file.path("..","output","root.voom5.cls"),append=TRUE)
```
...
# writing cls file
cat(paste(Total.leaf.plusR500,"2","1","\n"),file=file.path("..","output","leaf.voom5.cls"))
cat(paste(description,"\n"),file=file.path("..","output","leaf.voom5.cls"),append=TRUE)
cat(Live.Dead,file=file.path("..","output","leaf.voom5.cls"),append=TRUE)
```
...
# Loading leaf data 
```{r eval=FALSE, include=FALSE}
leaf_length <- read_csv(file.path("..","..","..","tools","leaf_data","leaf_lengths_combined.csv"))
...
write_csv(Br.v3.0anno.At.BLAST.highscore.modules2,path=file.path("..","output","WGCNA.modules2.voom5.csv.gz"))
```

# find genes in significantly correlated modules (FDR < 0.25) -> FDR < 0.3 (for root plum1 module) which is for GOseq ORA
```{r}
