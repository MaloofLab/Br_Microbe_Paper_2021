...
#
ElasticNet0.5_multiCV_Pct_var <- ElasticNet0.5_multiCV_Pct_var %>% unite(tissue.module,c("Module_Organ","Module_Name"),sep=".")
...
WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq.2.selected <- WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq.2 %>% 
  inner_join(ElasticNet0.5_multiCV_Pct_var, by=c("NAME"="tissue.module")) %>%
  arrange(Pct_Var_Explained) %>% drop_na(Pct_Var_Explained) #%>% View() # sorting modules
...
ggsave(plot.GSEA,filename="GSEA.RNAseq.plot.ML.full.png",path=file.path("RNA","output"),width=8,height=15)
...
#plot.combined
ggsave(plot.combined,path=file.path("RNA","output"),filename="WGCNA.voom5.leaf_avg_std.correlated.modules.ML.customcat.GSEA.combined.png",height=8,width=11)
# wider version
plot.combined <- plot_grid(plot.GSEA,WGCNA.voom5.all.modules.customcat.GOseq.plot.selected2,rel_widths=c(3,4),labels=c("A","B"))
#plot.combined
ggsave(plot.combined,path=file.path("RNA","output"),filename="WGCNA.voom5.leaf_avg_std.correlated.modules.customcat.MLGSEA.combined.wider.png",height=8,width=15)
...
GOseq.temp %>% enframe() %>% unnest(value) %>% write_csv(file.path("RNA","output","WGCNA.voom5.all.modules.GOseq.csv"))
...
ElasticNet0.5_multiCV_Pct_var %>% mutate(temp=str_remove_all(ME,"^ME")) %>% separate(temp,c("module","tissue"),sep="_") %>% mutate(tissue=str_c(tissue,".voom5.module")) %>% unite(tissue.module,tissue,module,sep="_") %>% inner_join(WGCNA.voom5.all.modules.GOseq,by=c("tissue.module"="name"))  %>% #View()
