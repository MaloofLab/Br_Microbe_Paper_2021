...
```{r}
GSEA_cust.root <- read_tsv(file.path("RNA","output","root.voom5.module.gmx")) %>% dplyr::slice(-1) # # remove row1 that has "na"
GSEA_cust.leaf <- read_tsv(file.path("RNA","output","leaf.voom5.module.gmx")) %>% dplyr::slice(-1) # # remove row1 that has "na"
...
genes.count.leaf <- GSEA_cust.leaf %>% 
  ...
  group_by(name) %>% drop_na() %>% summarise(n=n()) %>% 
  dplyr::rename(NAME=name)
genes.count <- bind_rows(genes.count.root,genes.count.leaf) %>% mutate(NAME=str_remove(NAME,".VOOM5.MODULE") %>% str_replace("ROOT","root") %>% str_replace("LEAF","leaf")) 
...
GSEA_reports.DF.plot.dendrogram.v9.WGCNAmodule.notitle.ML <- function(data = GSEA_reports.DF,
                                            ...
                                            leaf_avg_std.ML.data = ElasticNet0.5_LOO_prop_var,
                                            top.margin=830,
                                            bottom.margin=40,
                                            dendtrogram=c("TRUE", "FALSE"),
                                            legend.height=45,
                                            legend.width=25,
...
save(dendro.y.NAME,file=file.path("..","output","dendro.y.NAME.Rdata"))
...
# draw gene number inside heatmap
  ...
  geom_point(aes(x=0,y=y,size=n),color = "red",  inherit.aes = FALSE, data=genes.count2) +
      # for PropVar_0.5_LOO (120820)
      new_scale_fill() + # error
      geom_point(aes(x=0.35,y=y,fill=PropVar_0.5_LOO,shape=sign), size=3,inherit.aes = FALSE, data=genes.count2)  +
      scale_fill_gradient2(limit=c(-0.15,0.15), high=muted("magenta"),low=muted("green"),mid="white") +
      scale_shape_manual(values=c("positive"=24,"negative"=25)) + # trial to heve different shape 
...
```{r}
WGCNA.voom5.all.modules.customcat.GOseq <- read_csv(file.path("..","output","WGCNA.voom5.all.modules.customcat.GOseq.csv"))
#
...
#str(GSEA_reports.DF.formatted2) # not needed?

WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq <- WGCNA.voom5.all.modules.customcat.GOseq %>% mutate(name=str_replace(name,"_",".")) %>% filter(name %in% c(colnames(GSEA_cust.root),colnames(GSEA_cust.leaf))) %>% mutate(name=str_to_upper(name)) %>% dplyr::rename(NAME=name) %>% mutate(NAME=str_remove(NAME,"VOOM5.MODULE.") %>% str_replace("ROOT","root") %>% str_replace("LEAF","leaf")) # %>% mutate(NAME=factor(NAME,levels=levels(GSEA_reports.DF.formatted2$NAME)))
...
```{r}
# # count gene numbers in each category
# GSEA_cust.root <- read_tsv(file.path("..","output","root.voom5.moduleFDR0.3.gmx")) %>% dplyr::slice(-1) # # remove row1 that has "na"
# GSEA_cust.leaf <- read_tsv(file.path("..","output","leaf.voom5.moduleFDR0.3.gmx")) %>% dplyr::slice(-1) # # remove row1 that has "na"
# genes.count.root <- GSEA_cust.root %>% 
#   pivot_longer(cols=colnames(GSEA_cust.root)) %>% 
#   mutate(name=str_to_upper(name)) %>%
#   group_by(name) %>% drop_na() %>% summarise(n=n()) %>% 
...
# select modules
WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq.2 %>% View()
WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq.2.selected <- WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq.2 %>% 
  inner_join(ElasticNet0.5_LOO_prop_var, by=c("NAME"="tissue.module")) %>%
  arrange(PropVar_0.5_LOO) %>% drop_na(PropVar_0.5_LOO) #%>% View() # sorting modules
# which categories are exclused from WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq.2.selected
missing.categories <- ElasticNet0.5_LOO_prop_var %>% dplyr::select(tissue.module) %>% filter(!tissue.module %in% WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq.2.selected$NAME) %>% dplyr::rename(NAME=tissue.module)
...
WGCNA.voom5.all.modules.leaf_avg_std.correlated.customcat.GOseq.2.selected2 %>% group_by(NAME,category3.au.up_down) %>% summarize(n()) %>% View()

# relevel using dendrogram in GSEA plot
load(file.path("..","output","dendro.y.NAME.Rdata"))
...
ggsave(path=file.path("..","output"),filename="WGCNA.voom5.leaf_avg_std.correlated.modules.customcat.GOseq.4.selected.plot.png",height=8,width=11) 

```

# combine GSEA plot and WGCNA module ORA plot (under construction)
```{r}
# format GSEA plot
...
ggsave(plot.GSEA,filename="GSEA.RNAseq.plot.ML.full.png",path=file.path("..","output"),width=8,height=15)
...
ggsave(plot.GSEA,filename="GSEA.RNAseq.plot.ML.selected.png",path=file.path("..","output"),width=6,height=8)
...
#plot.combined
ggsave(plot.combined,path=file.path("..","output"),filename="WGCNA.voom5.leaf_avg_std.correlated.modules.ML.customcat.GSEA.combined.png",height=8,width=11)
# wider version
plot.combined <- plot_grid(plot.GSEA,WGCNA.voom5.all.modules.customcat.GOseq.plot.selected2,rel_widths=c(3,4),labels=c("A","B"))
#plot.combined
ggsave(plot.combined,path=file.path("..","output"),filename="WGCNA.voom5.leaf_avg_std.correlated.modules.customcat.MLGSEA.combined.wider.png",height=8,width=15)
...
load(file.path("..","..","..","Annotation","output","v3.0annotation","Brgo.v3.0anno.Atgoslim.BP.list.Rdata"))
```
...
```{r}
WGCNA.voom5.all.modules.GOseq <- read_csv(file.path("..","output","WGCNA.voom5.all.modules.GOseq.csv"))
WGCNA.voom5.all.modules.GOseq %>% dplyr::select(name)
ElasticNet0.5_LOO_prop_var %>% mutate(temp=str_remove_all(ME,"^ME")) %>% separate(temp,c("module","tissue"),sep="_") %>% mutate(tissue=str_c(tissue,".voom5.module")) %>% unite(tissue.module,tissue,module,sep="_") %>% inner_join(WGCNA.voom5.all.modules.GOseq,by=c("tissue.module"="name"))  %>% #View()
mutate(tissue.module=str_remove(tissue.module,".voom5.module")) %>% separate(tissue.module,into=c("tissue","module")) %>% dplyr::select(tissue,module,category,over_represented_pvalue,over_represented_padjust,term) %>% drop_na(category) %>% #View() 
  write_csv(file.path("..","output","WGCNA.voom5.growth.related.modules.GOseq.csv"))
```
