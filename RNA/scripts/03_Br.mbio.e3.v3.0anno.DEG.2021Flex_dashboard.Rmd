...
# combine
library(cowplot)
venn.ggplot <- venn.ggplot + theme_bw()
#library(ggplotify) # https://github.com/ggobi/ggally/issues/256
p.all <- cowplot::plot_grid(root.exp1and3.FDR0.1.plot.s, leaf.exp1and3.FDR0.1.plot.s, venn.ggplot, labels = c('A','B','C'), label_size = 9,ncol=2,rel_heights =c(1,1.5)) # ,rel_widths=c(1,1,2)
p.all <- p.all+ theme(plot.background = element_rect(fill="white")) # default background in plot C was gray.
ggsave2(p.all,filename=file.path("RNA","output","cor.plus.Venn3.png"),width=6.5,height=4,units="in") # width 6.5 (Feb 3, 2022)
ggsave2(p.all,filename=file.path("RNA","output","cor.plus.Venn3.pdf"),width=6.5,height=4,units="in")
