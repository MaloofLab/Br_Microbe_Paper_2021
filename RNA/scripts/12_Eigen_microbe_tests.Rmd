...
rootorder <- root_results %>% filter(fdr<0.1) %>% arrange(fdr) %>% pull(module)

root_results_full %>% filter(fdr<0.1) %>%
...
leaforder <- leaf_results %>% filter(fdr<0.1) %>% arrange(fdr) %>% pull(module)

leaf_results_full %>% filter(fdr<0.1) %>%
  select(module, data, fdr) %>% arrange(fdr) %>% unnest(data) %>%
  ggplot(aes(x=module, y=eigen_value, fill=trt)) +
  geom_boxplot() + 
