...
# 
genes.category.module <- Brgo.v3.0anno.Atgoslim.BP.list.DF %>% left_join(Br.v3.0anno.At.BLAST.highscore.modules.signedhybrid[,c("name","root.voom5.module","leaf.voom5.module")],by="name")
#
target.GO <- WGCNA.voom5.signedhybrid.all.modules.GOseq %>% dplyr::select(name,category,term) %>% filter(name=="leaf.red")
target.GO
#
target.genes <- genes.category.module %>% filter(leaf.voom5.module.signedhybrid=="maroon") %>% filter(value==target.GO$category[1]) %>% distinct(name)
target.genes
# expression pattern ~ soil treatment 
p.target <- plotgene(target=target.genes$name,numcol=8)
p.target
ggsave(p.target,filename="leaf.maroon.coregenes.yes.plot.png",path=file.path("RNA","output"),height=100,width=8,limitsize=FALSE) # soil treatment
```
...
# test 
mod=ElasticNet0.5_multiCV_Pct_var_signedhybrid[1]
...
# GO 1
target.genes <- genes.category.module %>% filter(leaf.voom5.module.signedhybrid==str_remove(mod,"leaf.")) %>% filter(value==target.GO$category[1]) %>% distinct(name)
