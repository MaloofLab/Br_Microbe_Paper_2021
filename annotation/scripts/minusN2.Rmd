---
title: "N_deficiency (minusN) responsive_genes 2"
output:
  html_document:
    df_print: paged
editor_options:
...
#### under construction #####
|NO3 starvation over 8 hrs|[Gutierrez et al. 2007](https://genomebiology.biomedcentral.com/articles/10.1186/gb-2007-8-1-r7#additional-information)|
roots and shoots of Col0, raised hydroponically w/ nutrients (w/ KNO3) for 6 weeks--at which point plants transfered to N free or kept in KNO3--tissue harvested 8hrs later.  Also a time course exp. that I think had the same treatments; however, plants were originally grown on MS salts, sucrose, & NH4; the time course sampling was 0.5, 1,2,4,8hrs after moving to -N or w/ NO3.|Microarray (ATH1 chip)|NOTE, all i can find here are genes and Pvalues--no ratios or FC.  You'd have to go to the RAW DATA to use this exp. IMO.  |[E-MEXP-828](https://www.ebi.ac.uk/arrayexpress/experiments/E-MEXP-828/)|
...
cel.filename2<-list.files(pattern=".CEL",path=file.path("","Volumes","data_work","Data8","Microarray","CN_Gutiérrez_2007","E-MEXP-828.raw.1"),full.names=TRUE) # "" is needed for file.path()!!!  with directory info
...
exp.info <- read_tsv(file=file.path("intermediate","minusN_Gutierrez_2007","E-MEXP-828.idf.txt")) # error

design <- exp.info %>% dplyr::select(`Array Data File`,`FactorValue[TREATMENT]`) %>%  separate(`FactorValue[TREATMENT]`,c("trt1","trt2"),sep=";") %>% group_by(trt1, trt2) %>% separate(`Array Data File`,c("name","id"),sep="-",remove=FALSE) %>% mutate(id=str_remove(id,"\\.CEL"))
design %>% View()
...
# combine
```{r eval=TRUE}
exp.values.combined <- exp.values %>% gather("cel_file","value",-element) %>% left_join(design,by=c("cel_file"="Array Data File")) %>% 
