...
# # dropna (how???)
# # consolidated.InX.table.dropna <- consolidated.InX.table %>% drop_na() # This is not what I want.
# #consolidated.InX.table %>% mutate(across(4:12,is.na)) %>% mutate(across(4:12,.fn=as.numeric)) %>% head() %>% rowwise() %>% mutate(is.na.num=sum(In01:In10orMore))  %>% View() # now what I want.
# temp <- consolidated.InX.table %>% mutate(across(2:12,is.na)) %>% mutate(across(2:12,.fn=as.numeric))
# temp2 <- temp %>% mutate(isnanum=rowSums(temp[,2:12])) %>% mutate(isnanum.module=rowSums(temp[,2:3])) #%>% View() # now this is what I want.
# temp2 %>% count(isnanum) # all NA is only 14058. "5" is 13778901.
# 
# dim(temp2)
# temp3 <- temp2 %>% filter(isnanum<9) %>% filter(isnanum.module < 2)
# dim(temp3)
# 
# # write csv
# write_csv(temp3,file=file.path("RNA","output","consolidated.InX.table.s.csv.gz")) # boo big. needs to drop off na line. J's suggestions is to use across() or accross_all() etc.
# 
# # read
# consolidated.InX.table <- read_csv(file.path("RNA","output","consolidated.InX.table.s.csv.gz"))
# dim(consolidated.InX.table) #[1] 16629963       14
# # a function to spit out overlapped gene names (wrong one. do no use it, 091421)
...
#   if(tissue=="root") {
# temp <- raw.table %>% filter(root.voom5.module.signedhybrid==module) %>% filter(!!as.name(InX)==!!as.name(category)) # this does not work
# print("print temp")
# print(temp)
# return(temp)
#   } else if(tissue=="leaf") {
...
# calc
root.In01 <- overlap.genes.WGCNAmodule.category2(custom.category.list=In01.consolidated.forORA.list2,tissue="root") %>% mutate(tissue="root",InX="In01") %>% rename(module=root.voom5.module.signedhybrid)
leaf.In01 <- overlap.genes.WGCNAmodule.category2(custom.category.list=In01.consolidated.forORA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="In01") %>% rename(module=leaf.voom5.module.signedhybrid)
root.In02orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In02orless.consolidated.forORA.list2,tissue="root") %>% mutate(tissue="root",InX="In02orless") %>% rename(module=root.voom5.module.signedhybrid)
leaf.In02orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In02orless.consolidated.forORA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="In02orless") %>% rename(module=leaf.voom5.module.signedhybrid)

root.In03orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In03orless.consolidated.forORA.list2,tissue="root") %>% mutate(tissue="root",InX="In03orless") %>% rename(module=root.voom5.module.signedhybrid)
leaf.In03orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In03orless.consolidated.forORA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="In03orless") %>% rename(module=leaf.voom5.module.signedhybrid)

root.In05orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In05orless.consolidated.forORA.list2,tissue="root") %>% mutate(tissue="root",InX="In05orless") %>% rename(module=root.voom5.module.signedhybrid)
leaf.In05orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In05orless.consolidated.forORA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="In05orless") %>% rename(module=leaf.voom5.module.signedhybrid)

root.In10orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In10orless.consolidated.forORA.list2,tissue="root") %>% mutate(tissue="root",InX="In10orless") %>% rename(module=root.voom5.module.signedhybrid)
leaf.In10orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In10orless.consolidated.forORA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="In10orless") %>% rename(module=leaf.voom5.module.signedhybrid)

root.In20orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In20orless.consolidated.forORA.list2,tissue="root") %>% mutate(tissue="root",InX="In20orless") %>% rename(module=root.voom5.module.signedhybrid)
leaf.In20orless <- overlap.genes.WGCNAmodule.category2(custom.category.list=In10orless.consolidated.forORA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="In20orless") %>% rename(module=leaf.voom5.module.signedhybrid)

root.Total <- overlap.genes.WGCNAmodule.category2(custom.category.list=Total.consolidated.forGSEA.list2,tissue="root") %>% mutate(tissue="root",InX="Total") %>% rename(module=root.voom5.module.signedhybrid)
leaf.Total <- overlap.genes.WGCNAmodule.category2(custom.category.list=Total.consolidated.forGSEA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="Total") %>% rename(module=leaf.voom5.module.signedhybrid)

root.In05orMore <- overlap.genes.WGCNAmodule.category2(custom.category.list=In05orMore.consolidated.forORA.list2,tissue="root") %>% mutate(tissue="root",InX="In05orMore") %>% rename(module=root.voom5.module.signedhybrid)
leaf.In05orMore <- overlap.genes.WGCNAmodule.category2(custom.category.list=In05orMore.consolidated.forORA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="In05orMore") %>% rename(module=leaf.voom5.module.signedhybrid)

root.In10orMore <- overlap.genes.WGCNAmodule.category2(custom.category.list=In10orMore.consolidated.forORA.list2,tissue="root") %>% mutate(tissue="root",InX="In10orMore") %>% rename(module=root.voom5.module.signedhybrid)
leaf.In10orMore <- overlap.genes.WGCNAmodule.category2(custom.category.list=In10orMore.consolidated.forORA.list2,tissue="leaf") %>% mutate(tissue="leaf",InX="In10oreMore") %>% rename(module=leaf.voom5.module.signedhybrid)
# combine all
...
# all in one plot (June 29, 2021) -> updated version (Sep 16, 2021 )
```{r}
InX.all.soilFC <- read_csv(file.path("RNA","output","InX.all.soilFC.csv"))
InX.comparison.plot4 <- function(data=data3,soilFC=InX.all.soilFC) {
  ...
  # plot 
WGCNA.voom5.signedhybrid.target.customcat.GOseq.plot.InXcomparison <- ggplot(data.combine, aes(x=category3.up_down.x,y=InX.x)) + geom_tile(aes(fill=minuslog10.soilFC.over_represented_padjust),colour="white") + scale_fill_gradient2(limit=c(-10,10), low="green",high=muted("magenta")) + facet_grid(NAME~tissue3.x,scales="free_x",space="free")
