---
title: "custom_categories_consolidate1_for_V5"
author: "Kazu"
date: "5/9/2021"
...
* Modify for updated plusN_Wang2003 (080621) by using v5
...
# read custom gene categories created in "06_d_cor_permutation.Rmd" -> needs to add this to this scripts -> use updated version (v4; 06**21) made by "custom_categories_v4_making.Rmd". Use root.all.comparison.v5.csv.gz & leaf.all.comparison.v5.csv.gz
```{r}
...
categories.up.down.combined <- categories.up.down %>% gsub("_up","",.) %>% gsub("_down","",.) %>% unique() # 30 categories. 29... why? (080721)
...
GSEA_cust_rootshoot_updown4.consolidated.FDR0.001.list <- GSEA_cust_rootshoot_updown2 %>% drop_na(value) %>% filter(str_detect(name,"FC")) %>% dplyr::select(-value) %>%split(.$genes) %>%   map(~dplyr::select(.,name)) %>% map(~as_vector(.))
save(GSEA_cust_rootshoot_updown4.consolidated.FDR0.001.list,file=file.path("annotation","output","cust_rootshoot_updown.v5.consolidated.FDR0.001.list.Rdata"))
...
# 
```{r}
load(file.path("annotation","output","cust_rootshoot_updown.v5.consolidated.FDR0.001.list.Rdata"))
...
filtered.custom.data.consolidated.numericL
save(filtered.custom.data.consolidated.numericL,file=file.path("RNA","output","filtered.custom.data.consolidated.numericL.Rdata"))
```
...
WGCNA.voom5.signedhybrid.target.customcat.GOseq.plot.InXcomparison <- WGCNA.voom5.signedhybrid.target.customcat.GOseq.plot.InXcomparison + theme(axis.text.x=element_text(size=12,angle=90,hjust=1,vjust=0.3),
                                      ...
                                      plot.title=element_text(size=14),
                                      axis.line=element_blank()) + 
      labs(x="",y="",fill="-log10\n FDR",title="WGCNA.customcat.consolidated.GOseq.plot.InXcomparison.allv2")
# adding overlap numbers
WGCNA.voom5.signedhybrid.target.customcat.GOseq.plot.InXcomparison <- WGCNA.voom5.signedhybrid.target.customcat.GOseq.plot.InXcomparison + geom_text(aes(label=numDEInCat))

WGCNA.voom5.signedhybrid.target.customcat.GOseq.plot.InXcomparison
ggsave(path=file.path("RNA","output"),filename=str_c("WGCNA.customcat.consolidated.GOseq.plot.InXcomparison.allv2.pdf"),height=200,width=20,limitsize=FALSE) 
}
# test
