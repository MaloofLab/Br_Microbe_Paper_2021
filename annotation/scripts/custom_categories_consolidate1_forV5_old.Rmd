...
# load consolidated custom categories (start here?)
```{r}
...
#
InX.comparison.plot1.v2 <- function(data=data3,target.module="root.voom5.module.signedhybrid.darkred",level.x = full.level.x, level.y=full.level.y) {# data3.root.darkred <- data3 %>% filter(NAME=="root.voom5.module.signedhybrid.darkred")
...
# make one function for one module (I am not satisfied with this function.)
```{r}
InX.comparison.plot6 <- function(data=data3,soilFC=InX.all.soilFC,which.module="root.voom5.module.signedhybrid.ivory") {
  data <- data %>% mutate(NAME2=str_remove_all(NAME,"voom5.module.signedhybrid.")) #%>% separate(NAME2,into=c("tissue","module"))
  # unite
    data <- data %>% unite(col=tissue.module.category.InX, c(NAME2,tissue3,category3.up_down,InX),sep=".",remove=FALSE) # this tissue3 is for category, not for module.
  # modify soilFC
    ...
    data.combine <- data.combine %>% mutate(minuslog10.soilFC.over_represented_padjust=ifelse(soilFC>=0,-log10(over_represented_padjust),log10(over_represented_padjust))) #%>% View()
    # select module
    data.combine <- data.combine %>% filter(NAME==which.module)
    # levels of categories
    levels(data.combine$category3.up_down.x)
...
# test
p.all3 <- InX.comparison.plot6()

# WGCNA modules
WGCNA.modules.all <- unique(data3$NAME) # old way
# 
data3 %>% distinct(NAME) # tidyverse way
# 
# map(data3 %>% distinct(NAME) ,function(x) InX.comparison.plot6(which.module=x)) # does not work
#
InX.heatmap.plot.all <- map(WGCNA.modules.all,function(x) InX.comparison.plot6(which.module=x)) # needs to have the same X axis. Revise InX.comparison.plot6()
#

ggsave(p.all3, path=file.path("RNA","output"),filename="WGCNA.customcatv5.consolidated1.GOseq.plot.InXcomparison.allmodules.soilFC.v3.pdf",height=800,width=20,limitsize=FALSE) 
```
...
# test
p.all <- InX.comparison.plot4()
#p.all
ggsave(p.all, path=file.path("RNA","output"),filename="WGCNA.customcatv5.consolidated1.GOseq.plot.InXcomparison.allmodules.soilFC.plot4.pdf",height=200,width=20,limitsize=FALSE) 
# this version is too long to find which column corresponds to which category.
```
