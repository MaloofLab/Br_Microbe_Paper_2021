...
```{r}
filtered.custom.data.consolidated %>% pivot_wider(names_from=category.cons, values_from = n) %>% 
  mutate(across(-genes, ~ ifelse(is.na(.), 0, 1))) %>%
  View() # why does not work?
```
...
InSet.percent.plot <- filtered.custom.data.summary %>%
  ...
  geom_point()  +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(InSet.percent.plot,path=file.path("annotation","output"),filename="InSet.percent.plot.withNishida2017.png",width=8,height=6)
```
...
# trial
In01.forGSEA %>% dplyr::select(1) %>% pivot_longer(1)
# 
In01.forGSEA %>% map(~pivot_longer(cols=name)) # does not work
##
In01.forGSEA.list <- list()
for(i in 1:(dim(In01.forGSEA)[2]-1)) {
  In01.forGSEA.list[[i]] <- In01.forGSEA %>% dplyr::select(i) %>% pivot_longer(1) %>% dplyr::rename(category=name)
}
#
In01.forGSEA.DF <- In01.forGSEA.list %>% enframe() %>% unnest(value) %>% drop_na(value)# does not work if I did not rename "category" columns
# convert into list by gene name
In01.forORA.list2 <- In01.forGSEA.DF %>%split(.$value) %>%   map(~dplyr::select(.,category)) %>% map(~as_vector(.))
save(In01.forORA.list2,file=file.path("annotation","output","In01.forORA.list2.Rdata"))
# ORA
# all modules
...
pivot_longer(cols=-1,names_to = "voom")  %>% drop_na(value) %>% unite(voom.module,c("voom","value")) %>%  split(.$voom.module) %>% map(~GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA2(genelist=.$name,custom.category.list=In01.forORA.list2,ontology="none")) # ,custom.category.list=GSEA_cust_rootshoot_updown2.list
GOseq.temp %>% enframe() %>% unnest(value) %>% write_csv(file.path("RNA","output","WGCNA.voom5.signedhybrid.all.modules.customcat.In01.FDR0.001.GOseq.csv"))

```

# In02.forGSEA
```{r}
In02orless.forGSEA <- filtered.custom.data.numericL %>%
  ...
  dplyr::select(-id) %>%
  mutate_all(sort, na.last=TRUE) 
In02orless.forGSEA <- In02orless.forGSEA[apply(In02orless.forGSEA,1,function(x) !all(is.na(x))),]
In02orless2.forGSEA %>% View()
##
In02orless.forGSEA.list <- list()
for(i in 1:(dim(In02orless.forGSEA)[2]-1)) {
  In02orless.forGSEA.list[[i]] <- In02orless.forGSEA %>% dplyr::select(i) %>% pivot_longer(1) %>% dplyr::rename(category=name)
}
#
In02orless.forGSEA.DF <- In02orless.forGSEA.list %>% enframe() %>% unnest(value) %>% drop_na(value)# does not work if I did not rename "category" columns
# convert into list by gene name
In02orless.forORA.list2 <- In02orless.forGSEA.DF %>%split(.$value) %>%   map(~dplyr::select(.,category)) %>% map(~as_vector(.))
save(In02orless.forORA.list2,file=file.path("annotation","output","In02orless.forORA.list2.Rdata"))

# ORA
# all modules
GOseq.temp <- Br.v3.0anno.At.BLAST.highscore.modules.signedhybrid %>% ungroup() %>% dplyr::select(name,root.voom5.module.signedhybrid,leaf.voom5.module.signedhybrid) %>%
pivot_longer(cols=-1,names_to = "voom")  %>% drop_na(value) %>% unite(voom.module,c("voom","value")) %>%  split(.$voom.module) %>% map(~GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA2(genelist=.$name,custom.category.list=In02orless.forORA.list2,ontology="none")) # ,custom.category.list=GSEA_cust_rootshoot_updown2.list
GOseq.temp %>% enframe() %>% unnest(value) %>% write_csv(file.path("RNA","output","WGCNA.voom5.signedhybrid.all.modules.customcat.In02orless.FDR0.001.GOseq.csv"))

```

# In03.forGSEA
```{r}
In03orless.forGSEA <- filtered.custom.data.numericL %>%
  ...
  dplyr::select(-id) %>%
  mutate_all(sort, na.last=TRUE) 
In03orless.forGSEA <- In03orless.forGSEA[apply(In03orless.forGSEA,1,function(x) !all(is.na(x))),]
In03orless.forGSEA %>% View()
##
In03orless.forGSEA.list <- list()
for(i in 1:(dim(In03orless.forGSEA)[2]-1)) {
  In03orless.forGSEA.list[[i]] <- In03orless.forGSEA %>% dplyr::select(i) %>% pivot_longer(1) %>% dplyr::rename(category=name)
}
#
In03orless.forGSEA.DF <- In03orless.forGSEA.list %>% enframe() %>% unnest(value) %>% drop_na(value)# does not work if I did not rename "category" columns
# convert into list by gene name
In03orless.forORA.list2 <- In03orless.forGSEA.DF %>%split(.$value) %>%   map(~dplyr::select(.,category)) %>% map(~as_vector(.))
save(In03orless.forORA.list2,file=file.path("annotation","output","In03orless.forORA.list2.Rdata"))

# ORA
# all modules
GOseq.temp <- Br.v3.0anno.At.BLAST.highscore.modules.signedhybrid %>% ungroup() %>% dplyr::select(name,root.voom5.module.signedhybrid,leaf.voom5.module.signedhybrid) %>%
pivot_longer(cols=-1,names_to = "voom")  %>% drop_na(value) %>% unite(voom.module,c("voom","value")) %>%  split(.$voom.module) %>% map(~GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA2(genelist=.$name,custom.category.list=In03orless.forORA.list2,ontology="none")) # ,custom.category.list=GSEA_cust_rootshoot_updown2.list
GOseq.temp %>% enframe() %>% unnest(value) %>% write_csv(file.path("RNA","output","WGCNA.voom5.signedhybrid.all.modules.customcat.In03orless.FDR0.001.GOseq.csv"))
...
##
In05orless.forGSEA.list <- list()
for(i in 1:(dim(In05orless.forGSEA)[2]-1)) {
  In05orless.forGSEA.list[[i]] <- In05orless.forGSEA %>% dplyr::select(i) %>% pivot_longer(1) %>% dplyr::rename(category=name)
}
#
In05orless.forGSEA.DF <- In05orless.forGSEA.list %>% enframe() %>% unnest(value) %>% drop_na(value)# does not work if I did not rename "category" columns
# convert into list by gene name
In05orless.forORA.list2 <- In05orless.forGSEA.DF %>%split(.$value) %>%   map(~dplyr::select(.,category)) %>% map(~as_vector(.))
save(In05orless.forORA.list2,file=file.path("annotation","output","In05orless.forORA.list2.Rdata"))

# ORA
# all modules
GOseq.temp <- Br.v3.0anno.At.BLAST.highscore.modules.signedhybrid %>% ungroup() %>% dplyr::select(name,root.voom5.module.signedhybrid,leaf.voom5.module.signedhybrid) %>%
pivot_longer(cols=-1,names_to = "voom")  %>% drop_na(value) %>% unite(voom.module,c("voom","value")) %>%  split(.$voom.module) %>% map(~GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA2(genelist=.$name,custom.category.list=In05orless.forORA.list2,ontology="none")) # ,custom.category.list=GSEA_cust_rootshoot_updown2.list
GOseq.temp %>% enframe() %>% unnest(value) %>% write_csv(file.path("RNA","output","WGCNA.voom5.signedhybrid.all.modules.customcat.In05orless.FDR0.001.GOseq.csv"))
...
##
In10orless.forGSEA.list <- list()
for(i in 1:(dim(In10orless.forGSEA)[2]-1)) {
  In10orless.forGSEA.list[[i]] <- In10orless.forGSEA %>% dplyr::select(i) %>% pivot_longer(1) %>% dplyr::rename(category=name)
}
#
In10orless.forGSEA.DF <- In10orless.forGSEA.list %>% enframe() %>% unnest(value) %>% drop_na(value)# does not work if I did not rename "category" columns
# convert into list by gene name
In10orless.forORA.list2 <- In10orless.forGSEA.DF %>%split(.$value) %>%   map(~dplyr::select(.,category)) %>% map(~as_vector(.))
save(In10orless.forORA.list2,file=file.path("annotation","output","In10orless.forORA.list2.Rdata"))

# ORA
# all modules
GOseq.temp <- Br.v3.0anno.At.BLAST.highscore.modules.signedhybrid %>% ungroup() %>% dplyr::select(name,root.voom5.module.signedhybrid,leaf.voom5.module.signedhybrid) %>%
pivot_longer(cols=-1,names_to = "voom")  %>% drop_na(value) %>% unite(voom.module,c("voom","value")) %>%  split(.$voom.module) %>% map(~GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA2(genelist=.$name,custom.category.list=In10orless.forORA.list2,ontology="none")) # ,custom.category.list=GSEA_cust_rootshoot_updown2.list
GOseq.temp %>% enframe() %>% unnest(value) %>% write_csv(file.path("RNA","output","WGCNA.voom5.signedhybrid.all.modules.customcat.In10orless.FDR0.001.GOseq.csv"))
```
...
#
In20orless.forGSEA.list <- list()
for(i in 1:(dim(In20orless.forGSEA)[2]-1)) {
  In20orless.forGSEA.list[[i]] <- In20orless.forGSEA %>% dplyr::select(i) %>% pivot_longer(1) %>% dplyr::rename(category=name)
}
#
In20orless.forGSEA.DF <- In20orless.forGSEA.list %>% enframe() %>% unnest(value) %>% drop_na(value)# does not work if I did not rename "category" columns
# convert into list by gene name
In20orless.forORA.list2 <- In20orless.forGSEA.DF %>%split(.$value) %>%   map(~dplyr::select(.,category)) %>% map(~as_vector(.))
save(In20orless.forORA.list2,file=file.path("annotation","output","In20orless.forORA.list2.Rdata"))

# ORA
# all modules
GOseq.temp <- Br.v3.0anno.At.BLAST.highscore.modules.signedhybrid %>% ungroup() %>% dplyr::select(name,root.voom5.module.signedhybrid,leaf.voom5.module.signedhybrid) %>%
pivot_longer(cols=-1,names_to = "voom")  %>% drop_na(value) %>% unite(voom.module,c("voom","value")) %>%  split(.$voom.module) %>% map(~GOseq.Brgo.v3.0.Atgoslim.BP.list.ORA2(genelist=.$name,custom.category.list=In20orless.forORA.list2,ontology="none")) # ,custom.category.list=GSEA_cust_rootshoot_updown2.list
GOseq.temp %>% enframe() %>% unnest(value) %>% write_csv(file.path("RNA","output","WGCNA.voom5.signedhybrid.all.modules.customcat.In20orless.FDR0.001.GOseq.csv"))
...
```{r}
filenames.csv <- list.files(path=file.path("RNA","output"),pattern="FDR0.001.GOseq.csv",full.names=TRUE,recursive=TRUE)
...
filenames.csv2 <- list.files(path=file.path("RNA","output"),pattern="FDR0.001.GOseq.csv",full.names=FALSE,recursive=FALSE)
# read xls file (file name is "xls", actually tsv. That is why read_xls did not work)
customcat.InX.FDR0.001.ORA <- lapply(filenames.csv, function(x) read_csv(file=file.path(x)))
# name
names(customcat.InX.FDR0.001.ORA) <- filenames.csv2
# convert list into one data.frame
customcat.InX.FDR0.001.ORA.DF <- customcat.InX.FDR0.001.ORA %>% enframe(name="InX.FDR0.001") %>% unnest(value)
```
...
```{r}
data <- customcat.InX.FDR0.001.ORA.DF %>% as_data_frame()
...
# Signature of WGCNA modules by looking enrichement of custom categories
```{r}
data2 <- data %>% mutate(name=str_replace(name,"_",".")) %>% dplyr::rename(NAME=name) %>%  mutate(category2=
...
data3 <- data2 %>% mutate(InX=str_replace(InX.FDR0.001,"WGCNA.voom5.signedhybrid.all.modules.customcat.FDR0.001.GOseq.csv","total")) %>% mutate(InX=str_remove_all(InX,"WGCNA.voom5.signedhybrid.all.modules.customcat.")) %>% mutate(InX=str_remove_all(InX,"\\.FDR0.001.GOseq.csv")) #%>% View() # does not work
data3 <- data3 %>% mutate(InX=factor(InX,levels=c("total","In20orless","In10orless","In05orless","In03orless","In02orless","In01")))
# select modules
...
####### under construction #######
# plot 
WGCNA.voom5.signedhybrid.root.ivory.customcat.GOseq.plot.InXcomparison <- ggplot(data3.root.ivory, aes(x=category3.au.up_down,y=InX)) + geom_tile(aes(fill=-log10(over_represented_padjust)),colour="white") + scale_fill_gradient2(limit=c(0,10), high=muted("magenta")) + facet_grid(.~tissue3,scales="free_x",space="free")
...
ggsave(path=file.path("RNA","output"),filename="WGCNA.voom5.signedhybrid.root.ivory.customcat.GOseq.plot.InXcomparison.png",height=8,width=11) 
```
