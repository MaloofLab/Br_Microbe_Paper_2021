...
#problems(x)
#spec_csv(x)
### there are problems in cold etc. (see duplicates obeject below)
filtered.custom.data %>% dplyr::select(matches("cold")) %>% View()
filtered.custom.data %>% filter(genes=="BraA04g024020.3C")
filtered.custom.data %>% filter(duplicated(genes))  # "BraA04g024020.3C" is duplicated! Why?
...
filtered.custom.data.TF6 %>% unite(genes.category, genes, category) %>% dplyr::count(genes.category) %>% arrange(desc(n)) # why five genes are duplicated?
duplicates <- filtered.custom.data.TF6 %>% unite(genes.category, genes, category) %>% dplyr::count(genes.category) %>% filter(n>1)
...
filtered.custom.data.TF7 %>% pivot_wider(names_from=category,values_from=value) # problem...
